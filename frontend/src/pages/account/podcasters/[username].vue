...
const isFormChanged = computed(() => {
  return (
    formData.value.username !== podcasterProfile.value?.username ||
    formData.value.email !== podcasterProfile.value?.email ||
...
const handleSubmit = async () => {
  ...
  if (formData.value.email !== podcasterProfile.value?.email) {
  ...
  if (formData.value.about !== podcasterProfile.value?.about) {
    updates.about = formData.value.about
  }
  if (formData.value.avatar_url !== podcasterProfile.value?.avatar_url) {
  ...
  if (Object.keys(updates).length > 0) {
    ...
    const { data } = await podcasterQuery({
      column: 'id',
      value: user.value.id,
    })
...
const cancelEditing = () => {
  ...
  formData.value = {
    username: podcasterProfile.value?.username || '',
    email: podcasterProfile.value?.email || '',
    about: podcasterProfile.value?.about || '',
    avatar_url: podcasterProfile.value?.avatar_url || '',
  }
  editMode.value = false
}
